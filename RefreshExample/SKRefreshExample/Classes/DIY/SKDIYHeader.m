...
- (void)placeSubviews
{
    ...
    self.logo.center = CGPointMake(self.sk_w*0.5, -self.logo.sk_h+20);
...
- (void)setState:(SKRefreshState)state
{
    ...
    switch (state) {
        case SKRefreshStateNormal:
            ...
            self.label.text = @"赶紧下拉吖(开关是打醋油滴)";
            break;
        case SKRefreshStatePulling:
            [self.loading stopAnimating];
            [self.s setOn:YES animated:YES];
            self.label.text = @"赶紧放开我把(开关是打醋油滴)";
            break;
        case SKRefreshStateRefreshing:
            [self.s setOn:YES animated:YES];
            self.label.text = @"加载数据中 (开关是打醋滴)";
...
#pragma mark---label的颜色随着下拉的比例而改变
- (void)setPullingPercent:(CGFloat)pullingPercent
{
    ...
    self.label.textColor = [UIColor colorWithRed:red green:green blue:blue alpha:1];
}
/*
